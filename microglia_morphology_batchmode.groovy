...
// Define result directory location
...
cells_min_area = 17.0         // Minimum area (in square microns) that an object must occupy to be considered a soma cell detection
...
cells_min_dab_OD_mean = 0.12  // The minimum required mean optical density (OD), i.e. average intensity of DAB staining within a detected cell
...
// Get the image name & strip off the date
imageName = GeneralTools.stripExtension(getProjectEntry().getImageName())
imageName = imageName.substring(11, imageName.length())

for (annotation_dv in getAnnotationObjects()) {
    println "========================================================================================================="
    
    // Create file name automatically from the image and annotation name 
    excel_save_name = "Microglia_Img["+ imageName +"]_region[" + annotation_dv.getName() + "]" // Name that's used for saving the excel table
    println "Starting process for image & region: ${excel_save_name}\n"
    ...
    selectObjects(annotation_dv)
    selected_annotation = annotation_dv
